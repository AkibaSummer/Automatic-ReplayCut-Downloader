...
async function main() {
  ...
  try {
    ...
    scheduled?.callback()
    await secondRefresh
    const metadataOnly = useAppStore.getState().clipTasks.find(task => task.id === created.taskId)
    assert.equal(metadataOnly?.status, 'done', 'hot task polling must not stat every historical output')
    assert.equal(metadataOnly?.file_path, outputPath)

    backend.db.healMissingClipFiles(backend.baseDir)
    const thirdRefresh = new Promise<void>(resolve => completedRefreshes.push(resolve))
    scheduled?.callback()
    await thirdRefresh
    ...
    assert.deepEqual(
      healed && { status: healed.status, progress: healed.progress, file_path: healed.file_path, message: healed.message },
      {
        ...
        message: `Output is currently unavailable; ownership path retained: ${outputPath}`,
      },
      'startup reconciliation must report an unavailable output without losing its ownership path',
    )
