...
class DesktopBackend {
  ...
  private async executeClip(rawUrl: string, startTime: number, endTime: number, qualityIndex: number = 0) {
    ...
    // Frame-accurate clipping via M4A sample table: stream-copy middle frames, boundary precision ±21ms
    ...
    if (sampleTable) {
      ...
      // Read sample rate from mp4a box in stsd
      ...
      if (mdatStart < 0) { fs.writeFileSync(outPath, buffer); return { path: outPath, fileName: path.basename(outPath), size: fs.statSync(outPath).size, title: info.title, duration: info.duration, startTime, endTime } }
      ...
      if (frames.length > 0) {
        ...
        muxer.finalize()
        fs.writeFileSync(outPath, Buffer.from(muxer.target.buffer as ArrayBuffer))
      } else {
